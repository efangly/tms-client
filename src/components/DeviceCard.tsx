...
export default function DeviceCard({ device, onEdit, scale = 1 }: DeviceCardProps) {
  ...
  const valueColor = isHumidity ? 'text-sky-500 dark:text-sky-400' : getTempColor(device.status);
  ...
  return (
    <div
      ...
      style={{ padding: px(14), borderRadius: px(10) }}
    >
      {/* Header */}
      <div className="flex items-center justify-between" style={{ marginBottom: px(10) }}>
        <div className="flex items-center flex-1 min-w-0" style={{ gap: px(8) }}>
          <div
            ...
            style={{ width: px(30), height: px(30), borderRadius: px(7) }}
          >
            {isHumidity ? (
              <svg style={{ width: px(16), height: px(16) }} className="text-sky-500 dark:text-sky-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 2C12 2 5 10 5 15a7 7 0 0014 0c0-5-7-13-7-13z" />
              </svg>
            ) : isThermometer ? (
              <svg style={{ width: px(16), height: px(16) }} className="text-orange-500 dark:text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <svg style={{ width: px(16), height: px(16) }} className="text-blue-600 dark:text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="min-w-0 flex-1">
            <h3
              ...
              style={{ fontSize: px(12.5), lineHeight: px(16) }}
            >
              {device.machineName}
            </h3>
          </div>
        </div>
        <div className="flex items-center shrink-0" style={{ gap: px(6) }}>
          <span
            ...
            style={{ gap: px(4), padding: `${px(3)} ${px(7)}`, fontSize: px(10.5) }}
          >
            <span className={`rounded-full ${statusStyle.dot}`} style={{ width: px(5), height: px(5) }}></span>
            {statusStyle.label}
          </span>
          {onEdit && (
            <button
      ...
      <div className="flex-1 flex items-center justify-center text-center">
        <div
          className={`font-bold ${valueColor} transition-colors duration-300`}
          style={{ fontSize: px(26), lineHeight: px(30) }}
        >
          {device.tempValue.toFixed(2)}
          <span style={{ fontSize: px(13), marginLeft: px(3) }}>{unit}</span>
        </div>
      </div>
      ...
      >
        <div className="flex items-center text-gray-400 dark:text-gray-500" style={{ gap: px(3), fontSize: px(10.5) }}>
          <svg style={{ width: px(12), height: px(12) }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
